...
def dense_flow(augs):
    ...
    video_path=os.path.join(src_path,video_name.split('_')[1],video_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--step',default=0,type=int,help='gap frames')
    ...
    flows_dirs = [video.split('.')[0] for video in vid_list]
    # pool.map(run_optical_flow, zip(vid_list, xrange(len(vid_list))))
    pool.map(dense_flow,zip(vid_list,flows_dirs,[step]*len(vid_list),[bound]*len(vid_list)))
